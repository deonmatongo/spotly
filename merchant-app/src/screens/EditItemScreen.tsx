...
import {
  ...
  KeyboardAvoidingView, Platform, Pressable, Switch, Alert,
} from 'react-native'
...
export default function EditItemScreen() {
  ...
  const { item } = useRoute<Route>().params
  ...
  const isDirty = name !== item.name || parseFloat(price) !== item.price
  ...
  return (
    <SafeAreaView style={styles.safe} edges={['top', 'bottom']}>
      <View style={styles.topBar}>
        <Pressable onPress={handleDiscard} hitSlop={12} style={[styles.backBtn, { borderColor: colors.border }]}>
          <Ionicons name="close" size={20} color={colors.textPrimary} />
        </Pressable>
        <AppText variant="h2" style={{ color: colors.textPrimary, flex: 1 }}>Edit item</AppText>
      </View>
      ...
      <View style={[styles.footer, { backgroundColor: colors.surface, borderTopColor: colors.border }]}>
        ...
        >
          ...
          <AppText variant="bodyBold" style={{ color: canSave ? '#fff' : colors.textLight, fontSize: 15 }}>Save changes</AppText>
        </Tappable>
      </View>
    </SafeAreaView>
  )
...
const makeStyles = (colors: Palette) => StyleSheet.create({
  ...
  input: { borderWidth: 1.3, borderRadius: 10, paddingHorizontal: 14, paddingVertical: 12, fontFamily: 'Manrope_500Medium', fontSize: 15 },
  ...
  saveBtn: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 10, paddingVertical: 16, borderRadius: 100 },
})
