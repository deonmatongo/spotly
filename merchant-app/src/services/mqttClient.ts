...
// Read-only trip subscriber (mirrors the customer app's client): jittered
// exponential-backoff reconnect + subscription replay, so callers subscribe
// once and ignore connection churn.
export class MqttConnection {
  ...
  constructor() {
    const clientId = `merchant-${Math.random().toString(36).slice(2, 10)}`
