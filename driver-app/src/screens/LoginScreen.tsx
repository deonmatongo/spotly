...
export default function LoginScreen({ onLogin }: Props) {
  ...
  const handleLogin = () => {
    if (username.trim() === 'tatendamoyo' && password === '123456') {
      onLogin()
    } else {
  ...
  return (
    <View style={styles.fill}>
      {/* Asymmetric colour wedges instead of a flat gradient panel */}
        ...
        <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={styles.fill}>
          <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scroll} keyboardShouldPersistTaps="handled">
            ...
            <Animated.View entering={FadeIn.delay(200).duration(400)}>
              <AppText variant="body" style={styles.tagline}>Sign in to start earning</AppText>
            </Animated.View>

            <Animated.View entering={FadeInDown.delay(260).duration(450).springify()}>
              <Animated.View style={[styles.form, shakeStyle]}>
                <FieldRow icon="person-outline" value={username} onChangeText={(t: string) => { setUsername(t); setError('') }} placeholder="Username" error={!!error} />
                <FieldRow
                ...
                <AppText variant="caption" style={styles.hintText}>Use tatendamoyo · 123456</AppText>
              </Animated.View>
            </Animated.View>

            <Animated.View entering={FadeIn.delay(500).duration(500)}>
              <AppText variant="label" style={styles.footer}>Move the city · earn your way</AppText>
            </Animated.View>
          </ScrollView>
...
const styles = StyleSheet.create({
  ...
  loginBtn: {
    backgroundColor: colors.primary, borderTopLeftRadius: 20, borderTopRightRadius: 6, borderBottomRightRadius: 20, borderBottomLeftRadius: 6,
